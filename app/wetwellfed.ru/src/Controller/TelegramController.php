...
class TelegramController extends AbstractController
{
    ...
    public function webhook(Security $security, Request $request, LoggerInterface $logger, EaterRepository $eaterRepository, ManagerRegistry $doctrine): Response
    {
        ...
        if ($data['message']['text'])
        {
            ...
            $authData = preg_split("/[ ]+/", $data['message']['text']);
            $logger->debug('$authData', $authData); //OR next($authData)
            $logger->debug('$data["message"]["text"]', [$data['message']['text']]); //OR next($authData)
            ...
            //  (в виде hash => user_id)
            if (isset($authData[1])){ // $authHash = $authData[1] = 65346331805633113aedb9d9f70334ef
                $authHash = $authData[1];
                $logger->debug('Telegram webhook response: HASH RECEIVED', [$authData[1]]); //OR next($authData)
...
//            "reply_to_message_id" => 7,
    ...
    /**
     * @Route("/test", name="tg_test")
     * @return void
     */
    public function test(Security $security, Request $request, EaterRepository $eaterRepository, MealRepository $mealRepository, ManagerRegistry $doctrine): void
    {
//        $this->sendMedia();
//        $this->sendFile();
//        die;
